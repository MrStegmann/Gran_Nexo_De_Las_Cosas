...
export const magicFlowGroup = new THREE.Group();
export const magicFlowNodes = [];
...
export function initMagicFlow() {
    magicFlowGroup.visible = false;
    magicFlowGroup.position.set(0, 0, 0);
    scene.add(magicFlowGroup);

    magicFlowGroup.add(filamentsGroup);
    ...
    const filamentMat = new THREE.MeshStandardMaterial({
        ...
        emissiveIntensity: 0.05, // Bajo para no tapar los nodos mágicos
        roughness: 0.1,
        metalness: 0.3,
        transparent: true,
        opacity: 0.85,
    ...
    function createNode(id, label, colorHex, x, y, z, hoverDesc) {
        ...
        group.userData = { id, type: 'magicFlow', desc: hoverDesc };

        // Núcleo Visual Mágico
        const coreMat = new THREE.MeshBasicMaterial({
            color: new THREE.Color(colorHex).multiplyScalar(80.0)
        });
        ...
        magicFlowNodes.push(nodeData);
    }

    // Nodos centrados perfectamente en las crestas de la curva base diagonal
    const posIntellect = baseCurve.getPointAt(0.42);
    const posWillpower = baseCurve.getPointAt(0.58);
    createNode('btn-intellect', 'Inteligencia', 0x00f0ff, posIntellect.x, posIntellect.y, posIntellect.z, "¡La zona de los listillos! Aquí hay Arcano, Vil, Naturaleza, Sombras y Nigromancia. ¡No te explotes el cerebro!");
    createNode('btn-willpower', 'Voluntad', 0xffd700, posWillpower.x, posWillpower.y, posWillpower.z, "¡Pura cabezonería mágica! Fe, Elune, Luz Sagrada, Elemental y Chi. ¡Paz mental y explosiones de luz!");

    // 3. Loop de Actualización Vivo
    addUpdatable((delta) => {
        const attributeSelection = document.getElementById('attribute-selection');
        const isActive = window.currentSection === 'hechizos' && attributeSelection && !attributeSelection.classList.contains('hidden');

        magicFlowGroup.visible = isActive;
        ...
        if (isActive) {
            ...
            const activeNodes = magicFlowNodes.filter(n => n.data.currentEmissive > 1.05);
